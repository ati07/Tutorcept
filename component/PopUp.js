...
function PopUp() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (
        (timerHours === 0 || timerHours === "00") &&
        (timerDays === 0 || timerDays === "00")
      ) {
        setShow(true);
        console.log("inside");
      } else {
  ...
  return (
    <div
      data-aos="zoom-in"
      className="flex flex-col justify-center rounded-3xl p-4 items-center lg:md:h-[300px] lg:md:w-[400px] h-auto w-[250px]  bg-[#473f3c6e] border-2"
    >
        {showEvents?
      ...
      <div className="text-white lg:md:h-[50px] grid grid-cols-12 lg:md:w-[400px] h-[25px] w-[300px] px-10 mt-5 pb-3">
        ...
        <div className="flex flex-col col-span-3 pt-1 text-2xl font-bold text-center border-2">
          {timerDays}
          <span className="text-sm font-light text-white"> Days</span>{" "}
        </div>
        <div className="flex flex-col col-span-1 pt-1 text-2xl text-center">
          :
        </div>
        <div className="flex flex-col col-span-2 pt-1 text-2xl font-bold text-center border-2">
          {timerHours}
          <span className="text-sm font-light text-white"> Hrs</span>{" "}
        </div>
        <div className="flex flex-col col-span-1 pt-1 text-2xl text-center">
          :
        </div>
        <div className="flex flex-col col-span-2 pt-1 text-2xl font-bold text-center border-2">
          {timerMinutes}
          <span className="text-sm font-light text-white">Mins </span>{" "}
        </div>
        <div className="col-span-1 pt-1 text-2xl text-center">:</div>
        <div className="flex flex-col col-span-2 pt-1 text-2xl font-bold text-center border-2">
          {timerSeconds}
          <span className="text-sm font-light text-white"> Secs </span>{" "}
        </div>
      </div>
        ...
        {show ? (
          <Link href="">
            ...
            <button
              ...
              className="hover:bg-green-500 bg-[#1e56a0] uppercase w-[200px] p-2 h-[50px] cursor-pointer rounded-lg opacity-100 hover:opacity-100 text-white"
            >
              Join the Room
            </button>
            {/* </a> */}
          ...
          <button
            onMouseEnter={enter}
            onMouseOut={close}
            className="hover:bg-green-500 bg-[#1e56a0] cursor-not-allowed uppercase w-[200px] p-2 h-[50px] rounded-lg opacity-100 hover:opacity-100 text-white"
          >
